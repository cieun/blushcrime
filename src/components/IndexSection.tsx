...
const IndexSection = ({ projects, onSelect }: IndexProps) => {
    ...
    const animate = () => {
      ...
      rects.forEach((r, i) => {
        ...
        if (i < projects.length) {
          const span = document.getElementById(`floating-item-${i}`);
          if (span) {
            span.style.transform = `translate3d(${r.x - r.w / 2}px, ${r.y - r.h / 2}px, 0)`;
          }
        } else {
          const indexBtn = document.getElementById("index-direction-mark");
          if (indexBtn) {
            indexBtn.style.transform = `translate3d(${r.x - r.w / 2}px, ${r.y - r.h / 2}px, 0)`;
          }
        }
      });
  ...
  }, [isIndexMode, projects, targetIndex]);
